...
const setPresence = async (client, num) => {
  switch (num) {
    case 0:
      await client.user
        .setActivity(
          botspeech.presenceSmiles.replace(
            /{{count}}/,
    ...
    case 1:
      await client.user
        .setActivity(
          botspeech.presenceChannels.replace(
            /{{count}}/,
    ...
    case 2:
      await client.user
        .setActivity(
          botspeech.presenceUsers.replace(/{{count}}/, client.users.cache.size),
          { type: "PLAYING" }
        )
        .then()
        .catch(console.error);
    ...
    case 3:
      await client.user
        .setActivity(botspeech.presenceInvite, { type: "LISTENING" })
        .then()
        .catch(console.error);
      return;
  }
