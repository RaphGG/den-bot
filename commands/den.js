...
const run = (client, message, args, settings) => {
  ...
  if (den) {
    ...
    if (pkmn.dens.sword.length == 0 && pkmn.dens.shield.length == 0) {
      message.channel
        .send(`**${pkmn.name}** is not in any current dens.`)
        .then()
        .catch(console.error);
      return;
    }

    const embed = embedHelper.createEmbed("denPkmn", client, [
      pkmnObj,
      pokedata.dens,
    ]);
    message.channel.send(embed).then().catch(console.error);
    ...
    message.channel.send(botspeech.denNoArg).then().catch(console.error);
    return;
  }
};
