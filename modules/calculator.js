...
const capProbRange = (pkmn, ball, gFlag, pFlag) => {
  ...
  if (isNaN(catchProb0) || isNaN(catchProb31)) return [0, 0];
...
const setModifiers = (pkmn) => {
  ...
  else pokedata.balls.find((x) => x.name == "Heavy Ball").setmod = 1;
...
exports.bestBalls = (pkmnObj) => {
  ...
  bestBalls.forEach((ball) => {
    ...
    const catchProb =
      catchProbRange[0] == catchProbRange[1]
        ? `\`${catchProbRange[0]}%\``
        : `\`${catchProbRange[0]}% ~ ${catchProbRange[1]}%\``;
    ...
    if (pkmnObj.promo) {
      ...
      const promoCatchProb =
        promoCatchRange[0] == promoCatchRange[1]
          ? `\`${promoCatchRange[0]}%\``
          : `\`${promoCatchRange[0]}% ~ ${promoCatchRange[1]}%\``;
...
exports.bestBall = (pkmnObj, ball) => {
  ...
  if (pkmnObj.promo) {
    ...
    pkmnObj.promoCatchProb =
      promoCatchProb[0] == promoCatchProb[1]
        ? `${promoCatchProb[0]}%`
        : `${promoCatchProb[0]}% ~ ${promoCatchProb[1]}%`;
  }
};
