...
let formsStr = "";
pokelists.noncosmeticforms.forEach(
  (form) => (formsStr += " " + form + "|" + form + " |")
);
...
const typeValues = (type1, type2) => {
  ...
  Object.keys(type1.atk).forEach((prop) => {
    const switchOn = type2
      ? type1.def[prop] * type2.def[prop]
      : type1.def[prop];
  ...
  atktimes4 =
    atktimes4 != ""
      ? `4x: \`${atktimes4.slice(0, atktimes4.lastIndexOf(", "))}\`\n`
      : "";

  atktimes2 =
    atktimes2 != ""
      ? `2x: \`${atktimes2.slice(0, atktimes2.lastIndexOf(", "))}\`\n`
      : "";
  ...
  atktimes0 =
    atktimes0 != ""
      ? `0x: \`${atktimes0.slice(0, atktimes0.lastIndexOf(", "))}\`\n`
      : "";

  atktimes05 =
    atktimes05 != ""
      ? `0.5x: \`${atktimes05.slice(0, atktimes05.lastIndexOf(", "))}\`\n`
      : "";

  atktimes025 =
    atktimes025 != ""
      ? `0.25x: \`${atktimes025.slice(0, atktimes025.lastIndexOf(", "))}\`\n`
      : "";

  atk2times4 =
    atk2times4 != ""
      ? `4x: \`${atk2times4.slice(0, atk2times4.lastIndexOf(", "))}\`\n`
      : "";

  atk2times2 =
    atk2times2 != ""
      ? `2x: \`${atk2times2.slice(0, atk2times2.lastIndexOf(", "))}\`\n`
      : "";
  ...
  atk2times0 =
    atk2times0 != ""
      ? `0x: \`${atk2times0.slice(0, atk2times0.lastIndexOf(", "))}\`\n`
      : "";

  atk2times05 =
    atk2times05 != ""
      ? `0.5x: \`${atk2times05.slice(0, atk2times05.lastIndexOf(", "))}\`\n`
      : "";

  atk2times025 =
    atk2times025 != ""
      ? `0.25x: \`${atk2times025.slice(0, atk2times025.lastIndexOf(", "))}\`\n`
      : "";

  deftimes4 =
    deftimes4 != ""
      ? `4x: \`${deftimes4.slice(0, deftimes4.lastIndexOf(", "))}\`\n`
      : "";

  deftimes2 =
    deftimes2 != ""
      ? `2x: \`${deftimes2.slice(0, deftimes2.lastIndexOf(", "))}\`\n`
      : "";
  ...
  deftimes0 =
    deftimes0 != ""
      ? `0x: \`${deftimes0.slice(0, deftimes0.lastIndexOf(", "))}\`\n`
      : "";

  deftimes05 =
    deftimes05 != ""
      ? `0.5x: \`${deftimes05.slice(0, deftimes05.lastIndexOf(", "))}\`\n`
      : "";

  deftimes025 =
    deftimes025 != ""
      ? `0.25x: \`${deftimes025.slice(0, deftimes025.lastIndexOf(", "))}\`\n`
      : "";
...
exports.createEmbed = (flag, client, args) => {
  ...
  if (flag == "top4") {
    ...
    const description = gmax
      ? `The best balls for catching G-Max ${pkmnObj.pkmn.name} are:`
      : `The best balls for catching ${pkmnObj.pkmn.name} are:`;
    ...
    if (pkmnObj.promo) embed.addField("Promo Top 4:", promoPercents, true);
    ...
    const promo = pkmnObj.promo
      ...
      : "";
    embed.setTitle(
      `Probability of catching ${gmax} ${pkmnObj.pkmn.name} with a ${ball.name} is: ${pkmnObj.catchProb}`
    );

    let emoji = "";

    if (ball.assumption == null) emoji = "❓";
    else emoji = ball.assumption ? "✅" : "⛔";
    ...
    // Types (Title)
    const type1 = client.emojis.cache.find(
      (x) => x.name == `Type${pkmn.type1}`
    );

    const type2 = client.emojis.cache.find(
      (x) => x.name == `Type${pkmn.type2}`
    );
    ...
    const titleUrl =
      pkmn.generation == "SwordShield"
        ? `https://serebii.net/pokedex-swsh/${pkmnNoForm.toLowerCase()}/`
        : `https://serebii.net/pokedex-sm/${dexId}.shtml`;
    ...
    if (pkmn.eggGroup2) egg = egg + `, ${pkmn.eggGroup2}\``;
    else egg = egg + `\``;
    ...
    const baseStats1 = `\`${pkmn.baseStats.hp
      .toString()
      .padEnd(7, " ")}${pkmn.baseStats.atk
      .toString()
      .padEnd(8, " ")}${pkmn.baseStats.def.toString().padEnd(3, " ")}\``;

    const baseStats2 = `\`${pkmn.baseStats.spA
      .toString()
      .padEnd(7, " ")}${pkmn.baseStats.spD
      .toString()
      .padEnd(8, " ")}${pkmn.baseStats.spe.toString().padEnd(3, " ")}\``;
    ...
    if (dens != "") embed.addField("Dens", dens, false);
    ...
    embed.setImage(
      `https://raphgg.github.io/den-bot/data/dens/den${args.den}.png`
    );

    embed.setTitle(`Den ${args.den}:`);
    embed.setURL(
      `https://www.serebii.net/swordshield/maxraidbattles/den${args.den}.shtml`
    );
    const swordHa = args.sword
      .filter((pkmn) => {
    ...
    if (swordHa.length > 1) embed.addField("Sword HA:", swordHa, true);

    if (shieldHa.length > 1) embed.addField("Shield HA:", shieldHa, true);
    ...
    const shieldFltr = denArr.filter((den) =>
      pkmn.dens.shield.includes(den.den)
    );
    ...
    if (gmax) {
      shieldArr = shieldFltr
        .filter((den) =>
          den.shield.some((mon) => mon.name == pkmn.name && mon.gigantamax)
        )
        .map((den) => den.den);

      swordArr = swordFltr
        .filter((den) =>
          den.sword.some((mon) => mon.name == pkmn.name && mon.gigantamax)
        )
        .map((den) => den.den);
      ...
      shieldHa = shieldFltr
        .filter((den) =>
          den.shield.some(
            (mon) => mon.name == pkmn.name && mon.ability.startsWith("Hidden")
          )
        )
        .map((den) => den.den);
    ...
    if (pkmn.dens.sword.length > 0) {
      ...
      swordArr.forEach((den) => {
        swordDens +=
          `[${den}](https://www.serebii.net/swordshield/maxraidbattles/den${den}.shtml)` +
          ", ";
      });
    ...
    const ballurl = `https://serebii.net/itemdex/${args.name
      .replace(" ", "")
      .toLowerCase()}.shtml`;
    ...
    const url = `https://raphgg.github.io/den-bot/data/sprites/balls/${args.name
      .replace(/ ball/gi, "")
      .toLowerCase()}.png`;
    embed.setThumbnail(url);

    const aniUrl = `https://raphgg.github.io/den-bot/data/sprites/balls/${args.name
      .replace(/ ball/gi, "")
      .toLowerCase()}.gif?cache=42069`;
    ...
    embed.setTitle(title);
    embed.setURL(
      `https://bulbapedia.bulbagarden.net/wiki/${type1.name}_(type)`
    );
    ...
    embed.setThumbnail(
      "https://raphgg.github.io/den-bot/data/readme/froslass-credits.gif"
    );
    ...
    embed.setTimestamp();
    embed.setThumbnail(
      "https://raphgg.github.io/den-bot/data/readme/alcremieinvite.gif"
    );
    ...
    embed.addField(
      "Pokémon Commands:",
      botspeech.pokeCommands.replace(/{{prefix}}/g, args)
    );

    embed.addField(
      "User Commands:",
      botspeech.userCommands.replace(/{{prefix}}/g, args)
    );

    embed.addField(
      "Admin & Guild Commands:",
      botspeech.adminCommands.replace(/{{prefix}}/g, args)
    );
    ...
    embed.addField(
      "Examples:",
      cmd.example.replace(/{{prefix}}/gi, prefix),
      true
    );
