...
exports.run = async (client, message, args) => {
  ...
  if (!ownerOrAdmin) return message.reply(botspeech.permNotFound);
  else if (!args || args.length < 1)
    return message.channel.send(botspeech.pingNoArg);
  else {
    const pingroles = settings.roles.pingroles
      .map((role) => role.name)
      .join(", ");

    const rolename = args.join(" ");

    const pingrole = settings.roles.pingroles.find((role) =>
      startsOrIs(role.name, rolename)
    );

    if (!pingrole)
      return message.channel.send(
        botspeech.roleNotFound.replace(`{{roles}}`, pingroles)
      );
    ...
    guildrole
      .setMentionable(true, "Role to be pinged.")
      .then((updated) => {
        guild
          .fetchMember(message.author.id)
          .then((member) =>
            message.channel.send(`${updated} *(requested by ${member})*`)
          )
      ...
      .catch((error) => {
        ...
        if (error.message == "Missing Permissions")
          return message.channel.send(
            botspeech.rolePlacement.replace(/{{role}}/gi, guildrole.name)
          );
        else return message.channel.send(botspeech.pingError);
      });
  }
};
